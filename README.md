...
- **Should**: Android, iOS, and MAUI .NET 6 workloads (use `dotnet workload install android ios maccatalyst maui` to install them)
  (Seems iOS and Mac Catalyst workloads are only supported on Windows or macOS. So if you're not using either Windows or macOS, exclude `ios maccatalyst` from the command)
